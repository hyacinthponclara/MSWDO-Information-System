...
$programDB = $pdo->prepare("
    ...
    FROM PROGRAM
    WHERE program_name = 'Senior Citizen Program'
    LIMIT 1
");
$programDB->execute();
...
$progID_DB = $pdo->prepare("SELECT program_id FROM PROGRAM WHERE program_name = 'Senior Citizen Program' LIMIT 1");
$progID_DB->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_fund'])) {
    ...
    $senior_status = trim($_POST['seniorStatus'] ?? 'Active');
    ...
    $upload_dir = __DIR__ . '/uploads/senior/';
        ...
        if (!empty($_FILES[$key]['name'])) {
            ...
            if (!in_array($ftype, $allowed_types)) {
                ...
                if (move_uploaded_file($_FILES[$key]['tmp_name'], $upload_dir . $fname)) {
                    $doc_paths[$key] = 'uploads/senior/' . $fname;
                } else {
    ...
    if (empty($errors)) {
        try {
            ...
            $fpDB = $pdo->prepare("
                INSERT INTO senior
                    (availment_id, user_id, senior_household_num, senior_status)
                VALUES (?, ?, ?, ?)
            ");
            $fpDB->execute([
                $availment_id,
...
<body class="bg-slate2 min-h-screen flex">
            ...
            <div class="max-w-3xl mx-auto space-y-5">
                ...
                <div class="animate-fade-up">
                    ...
                    <p class="text-[13px] text-slate-500 mt-1">
                        Request funds for senior citizen events and programs. &mdash; <span
                ...
                <?php if ($success === 'submitted'): ?>
                    <div
                        ...
                        <span>Senior activity fund request submitted successfully! It is now pending approval.</span>
                    </div>
                ...
                <form method="POST" enctype="multipart/form-data" id="fundForm">
                    ...
                    <div class="animate-fade-up-2 bg-white rounded-2xl border border-slate-200 overflow-hidden mb-5">
                        ...
                        <div class="p-6 space-y-5">
                            ...
                            <!-- Row 4: Senior-specific fields -->
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="field-label">Household Number</label>
                                    <input type="text" name="householdNum" class="field"
                                        placeholder="Senior Household Number (optional)"
                                        value="<?= htmlspecialchars($_POST['householdNum'] ?? '') ?>">
                                </div>
                                <div>
                                    <label class="field-label req">Senior Status</label>
                                    <select name="seniorStatus" class="field">
                                        <option value="Active" <?= (($_POST['seniorStatus'] ?? 'Active') === 'Active') ? 'selected' : '' ?>>Active</option>
                                        <option value="Graduated" <?= (($_POST['seniorStatus'] ?? '') === 'Graduated') ? 'selected' : '' ?>>Graduated</option>
                                        <option value="Suspended" <?= (($_POST['seniorStatus'] ?? '') === 'Suspended') ? 'selected' : '' ?>>Suspended</option>
                                    </select>
                                </div>
                            </div>
    ...
    <script>
        ...
        <?php if ($success === 'submitted'): ?>
            window.addEventListener('DOMContentLoaded', () => showToast('Senior activity fund request submitted ✓'));
        <?php elseif ($success === 'draft'): ?>
