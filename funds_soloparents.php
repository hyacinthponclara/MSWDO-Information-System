...
$programDB = $pdo->prepare("
    ...
    FROM PROGRAM
    WHERE program_name = 'Solo Parent Program'
    LIMIT 1
");
$programDB->execute();
...
$progID_DB = $pdo->prepare("SELECT program_id FROM PROGRAM WHERE program_name = 'Solo Parent Program' LIMIT 1");
$progID_DB->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_fund'])) {
    ...
    $soloparent_status = trim($_POST['soloparentStatus'] ?? 'Active');
    ...
    $upload_dir = __DIR__ . '/uploads/soloparents/';
        ...
        if (!empty($_FILES[$key]['name'])) {
            ...
            if (!in_array($ftype, $allowed_types)) {
                ...
                if (move_uploaded_file($_FILES[$key]['tmp_name'], $upload_dir . $fname)) {
                    $doc_paths[$key] = 'uploads/soloparents/' . $fname;
                } else {
    ...
    if (empty($errors)) {
        try {
            ...
            // Insert into soloparents
            $fpDB = $pdo->prepare("
                INSERT INTO soloparents
                    (availment_id, user_id, soloparent_household_num, soloparent_status)
                VALUES (?, ?, ?, ?)
            ");
            $fpDB->execute([
                $availment_id,
...
<head>
    ...
    <title>Solo Parent Fund Request – MSWDO San Enrique</title>
...
<body class="bg-slate2 min-h-screen flex">
    ...
    <div class="ml-64 flex-1 flex flex-col min-h-screen">
        ...
        <header
            ...
            <div class="flex items-center gap-2 text-[13px]">
                ...
                <span class="text-slate-300">/</span>
                <span class="text-navy-600 font-semibold">Solo Parent Activity Fund Request</span>
            </div>
        ...
        <main class="p-6 overflow-y-auto">
            <div class="max-w-3xl mx-auto space-y-5">
                ...
                <div class="animate-fade-up">
                    <h1 class="text-xl font-serif text-navy-600">Solo Parent Activity Fund Request</h1>
                    <p class="text-[13px] text-slate-500 mt-1">
                        Request funds for Solo Parent events and programs. &mdash; <span
                            class="font-semibold text-navy-600"><?= $barangay_name ?></span>
                    </p>
                </div>
                ...
                <?php if ($success === 'submitted'): ?>
                    <div
                        ...
                        <span>Solo Parent activity fund request submitted successfully! It is now pending approval.</span>
                    </div>
                ...
                <form method="POST" enctype="multipart/form-data" id="fundForm">
                    ...
                    <div class="animate-fade-up-2 bg-white rounded-2xl border border-slate-200 overflow-hidden mb-5">
                        ...
                        <div class="p-6 space-y-5">
                            ...
                            <!-- Row 4: Solo Parent-specific fields -->
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="field-label">Household Number</label>
                                    <input type="text" name="householdNum" class="field"
                                        placeholder="Solo Parent Household Number (optional)"
                                        value="<?= htmlspecialchars($_POST['householdNum'] ?? '') ?>">
                                </div>
                                <div>
                                    <label class="field-label req">Solo Parent Status</label>
                                    <select name="soloparentStatus" class="field">
                                        <option value="Active" <?= (($_POST['soloparentStatus'] ?? 'Active') === 'Active') ? 'selected' : '' ?>>Active</option>
                                        <option value="Graduated" <?= (($_POST['soloparentStatus'] ?? '') === 'Graduated') ? 'selected' : '' ?>>Graduated</option>
                                        <option value="Suspended" <?= (($_POST['soloparentStatus'] ?? '') === 'Suspended') ? 'selected' : '' ?>>Suspended</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="bg-slate-50 border border-slate-200 rounded-xl overflow-hidden" id="limitPanel">
                                <div
                                    ...
                                    <p class="text-[11px] font-semibold text-navy-600">Automatic Budget Check — Solo Parent
                                        Activity Fund</p>
    ...
    <script>
        ...
        <?php if ($success === 'submitted'): ?>
            window.addEventListener('DOMContentLoaded', () => showToast('Solo Parent activity fund request submitted ✓'));
        <?php elseif ($success === 'draft'): ?>
