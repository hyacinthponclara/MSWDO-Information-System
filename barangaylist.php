...
<head>
  ...
  <style>
    ...
    .barangay-row { transition:all .15s ease; cursor:pointer; }
...
<body class="bg-slate2 min-h-screen flex">
  ...
  <div class="ml-64 flex-1 flex flex-col min-h-screen">
    ...
    <main class="flex-1 p-6 flex flex-col gap-6">
      ...
      <!-- BARANGAY LIST with ID + Total Availments columns -->
      <div class="animate-fade-up-1 bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
        <div class="flex items-center justify-between px-5 py-3.5 border-b border-slate-100 bg-slate-50/60">
          <p class="text-[13px] font-semibold text-navy-600"><i class="fas fa-list-ul mr-2 text-navy-400"></i>All Barangays</p>
          <span class="text-[11px] text-slate-400 bg-white/60 px-2 py-0.5 rounded-full">10 records</span>
        </div>
        
        <!-- Table: Barangay ID | Barangay Name | Total Availments | Request Fund -->
        <table class="w-full text-[13px]">
          <thead>
            <tr class="border-b border-slate-100 bg-slate-50/30">
              <th class="px-5 py-3 text-left"><span class="text-[10px] font-bold uppercase tracking-wider text-slate-500">Barangay ID</span></th>
              <th class="px-5 py-3 text-left"><span class="text-[10px] font-bold uppercase tracking-wider text-slate-500">Barangay Name</span></th>
              <th class="px-5 py-3 text-right"><span class="text-[10px] font-bold uppercase tracking-wider text-slate-500">Total Availments</span></th>
              <th class="px-5 py-3 text-center"><span class="text-[10px] font-bold uppercase tracking-wider text-slate-500">Request Fund</span></th>
            </tr>
          </thead>
          <tbody id="barangayTableBodyClean">
            <!-- Dynamic rows: ID, Name, Total Availments, Request button -->
          </tbody>
        </table>
      </div>
    </main>
  ...
  <script>
    const barangayData = [
      { id: "BRG-001", name: "Bagonawa",      totalAvailments: 0 },
      { id: "BRG-002", name: "Baliwagan",     totalAvailments: 0 },
      { id: "BRG-003", name: "Batuan",        totalAvailments: 0 },
      { id: "BRG-004", name: "Guintorilan",   totalAvailments: 0 },
      { id: "BRG-005", name: "Nayon",         totalAvailments: 0 },
      { id: "BRG-006", name: "Poblacion",     totalAvailments: 0 },
      { id: "BRG-007", name: "Sibucao",       totalAvailments: 0 },
      { id: "BRG-008", name: "Tabao Baybay",  totalAvailments: 0 },
      { id: "BRG-009", name: "Tabao Rizal",   totalAvailments: 0 },
      { id: "BRG-010", name: "Tibsoc",        totalAvailments: 0 }
    ];
    ...
    // Render table with Barangay ID, Name, Total Availments, and Request button
    function renderBarangayTable() {
      const tbody = document.getElementById('barangayTableBodyClean');
      if (!tbody) return;

      tbody.innerHTML = barangayData.map(barangay => {
        const requestUrl = `barangay_fund_request.php?barangay=${encodeURIComponent(barangay.name)}&id=${encodeURIComponent(barangay.id)}`;
        
        return `
          <tr class="barangay-row border-b border-slate-100 group">
            <td class="px-5 py-4">
              <div class="flex items-center gap-2">
                <span class="text-[12px] font-mono font-medium text-navy-500 bg-navy-50 px-2 py-0.5 rounded-md">${barangay.id}</span>
              </div>
            </td>
            <td class="px-5 py-4">
              <div class="flex items-center gap-3">
                <span class="font-semibold text-navy-700 text-[14px] tracking-tight">${barangay.name}</span>
                <i class="fas fa-arrow-right row-arrow text-navy-400 text-[11px] ml-1"></i>
              </div>
            </td>
            <td class="px-5 py-4 text-right">
              <div class="flex items-center justify-end gap-2">
                <span class="font-bold text-navy-700 bg-navy-50 px-2.5 py-1 rounded-lg text-[13px] inline-block">${formatCurrency(barangay.totalAvailments)}</span>
              </div>
            </td>
