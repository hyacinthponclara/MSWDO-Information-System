...
$stmt = $pdo->query("
    SELECT user_id, username, user_password,
           ...
           user_role, user_position, user_employment_status,
           user_office, user_municipality, user_province,
           user_date_joined, user_email, user_contactnum,
           user_isactive, user_last_login
    FROM mswdo_user
...
$usersData = array_map(function ($u) {
    return [
        'id'                 => (int) $u['user_id'],
        'accountId'          => 'MSWDO-USER-' . str_pad((string) $u['user_id'], 3, '0', STR_PAD_LEFT),
        ...
        'employmentStatus'   => $u['user_employment_status'],
        'office'             => $u['user_office'],
        'municipality'       => $u['user_municipality'],
        'province'           => $u['user_province'],
        'dateJoined'         => $u['user_date_joined']
            ? date('F d, Y · h:i A', strtotime($u['user_date_joined']))
            : '—',
        'email'              => $u['user_email'],
        'contact'            => $u['user_contactnum'],
        'isActive'           => (bool) $u['user_isactive'],
        'lastLogin'          => $u['user_last_login']
            ? date('F d, Y · h:i A', strtotime($u['user_last_login']))
            : 'Never',
    ];
    ...
    <div class="md:ml-64 flex-1 flex flex-col min-h-screen w-full">
                        ...
                        <div class="p-5 space-y-6 overflow-y-auto max-h-[70vh]">
                            ...
                            <section class="pt-5 border-t border-slate-100">
                                ...
                                <div class="mt-4">
                                    ...
                                    <p id="roleHelp" class="text-[10px] text-slate-400 mt-1">
        ...
        <script>
            ...
            function openAddModal() {
                ...
                updateRoleEditRestriction(null);
            ...
            /* ROLE EDIT RESTRICTION */

            function updateRoleEditRestriction(existingRole = null) {
                ...
                if (existingRole === null) {
                    ...
                    roleHelp.textContent =
                        "Choose Admin or Staff. To promote a Staff member to Admin, create a new Admin account.";
                    return;
                }
                ...
                if (existingRole === "Staff") {
                    ...
                    roleHelp.textContent =
                        "This Staff account cannot be changed to Admin. Create a new Admin account for the employee.";
                } else {
                    adminOption.disabled = false;
                    roleHelp.textContent =
                        "Role controls the user's permissions. Staff → Admin requires a new Admin account.";
                }
            }
            ...
            function editUser(id) {
                ...
                document.getElementById("accountId")
                    .value =
                    user.accountId || `MSWDO-USER-${String(user.id).padStart(3, "0")}`;
                ...
                updateRoleEditRestriction(user.role);
            ...
            function saveUser(event) {
                ...
                const employmentStatus = getValue("employmentStatus");
                const office = "MSWDO";
                ...
                if (!firstName) {
                    showValidation("firstName", "Please enter the first name.");
                    isValid = false;
                } else if (!isValidName(firstName)) {
                    showValidation("firstName", "First name may only contain letters, spaces, hyphens, or apostrophes.");
                    isValid = false;
                }

                if (!lastName) {
                    showValidation("lastName", "Please enter the last name.");
                    isValid = false;
                } else if (!isValidName(lastName)) {
                    showValidation("lastName", "Last name may only contain letters, spaces, hyphens, or apostrophes.");
                    isValid = false;
                }

                if (!username) {
                    showValidation("username", "Please enter a username.");
                    isValid = false;
                } else if (!/^[A-Za-z0-9._-]{4,30}$/.test(username)) {
                    showValidation("username", "Username must be 4–30 characters and may contain letters, numbers, dot, underscore, or hyphen.");
                    isValid = false;
                }

                const isAdding = currentEditId === null;

                // Existing Staff accounts cannot be promoted to Admin.
                if (!isAdding) {
                    const existingUser = usersData.find(
                        item => Number(item.id) === Number(currentEditId)
                    );
                    ...
                    if (
                        ...
                        showValidation(
                            "role",
                            "A Staff account cannot be changed to Admin. Create a new Admin account instead."
                        );
                        showToast(
                            "Staff → Admin requires a new Admin account.",
                            "error"
                        );
                        return;
                    }
                ...
                if (isAdding && !password) {
                    showValidation("password", "Password is required when creating a new user.");
                    isValid = false;
                }

                if (password) {
                    if (password.length < 8) {
                        showValidation("password", "Password must contain at least 8 characters.");
                        isValid = false;
                    }

                    if (password !== confirmPassword) {
                        showValidation("confirmPassword", "Passwords do not match.");
                        isValid = false;
                    }
                }

                if (isAdding && !confirmPassword) {
                    showValidation("confirmPassword", "Please confirm the password.");
                    isValid = false;
                }

                if (!role) {
                    showValidation("role", "Please select a system role.");
                    isValid = false;
                }

                if (!email) {
                    showValidation("email", "Please enter an email address.");
                    isValid = false;
                ...
                if (!contact) {
                    showValidation("contact", "Please enter a contact number.");
                    isValid = false;
                } else if (!isValidPhilippineContact(contact)) {
                    showValidation("contact", "Enter a valid Philippine mobile number, e.g. 09123456789.");
                    isValid = false;
                }

                if (!position) {
                    showValidation("position", "Please enter the employee's position.");
                    isValid = false;
                }

                if (!employmentStatus) {
                    showValidation("employmentStatus", "Please select the employment status.");
                    isValid = false;
                }

                if (!office) {
                    showValidation("office", "Please enter the office.");
                    isValid = false;
                }

                if (!municipality) {
                    showValidation("municipality", "Please enter the municipality.");
                    isValid = false;
                }

                if (!province) {
                    showValidation("province", "Please enter the province.");
                    isValid = false;
                }
                ...
                saveBtnText.textContent = isAdding ? "Saving..." : "Updating...";

                const formData = new FormData();

                formData.append("action", isAdding ? "add" : "edit");
                formData.append("id", isAdding ? "0" : String(currentEditId));
                ...
                fetch("usermanagement_action.php", {
                    method: "POST",
                    body: formData,
                    ...
                    .then(response => {
                        ...
                        return response.text().then(text => {
                            throw new Error(text || "Unable to save the user.");
                        });
                    })
                    .catch(error => {
                        console.error(error);
                        ...
                        saveBtnText.textContent = isAdding ? "Save User" : "Save Changes";
                        showToast("Unable to save the user. Please try again.", "error");
                    });
            }
            ...
            function confirmToggleStatus(id) {
                ...
                const user = usersData.find(item => Number(item.id) === Number(id));
                ...
                document.getElementById("confirmActionButton").onclick = function () {
                    ...
                    fetch("usermanagement_action.php", {
                        ...
                        .then(response => {
                            ...
                            return response.text().then(text => {
                                throw new Error(text || "Unable to update account status.");
                            });
                        })
                        .catch(error => {
                            console.error(error);
