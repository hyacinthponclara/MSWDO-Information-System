...
<body class="bg-slate2 min-h-screen flex">
        ...
        <main class="p-6 overflow-y-auto">
            <div class="max-w-3xl mx-auto space-y-5">
                ...
                <div
                    ...
                    <p class="text-[12px] text-navy-700">SFP is recorded <strong class="font-semibold">per child, per
                            feeding
                            cycle</strong>. Submit a separate record for each child.</p>
                </div>
                ...
                <div class="animate-fade-up-2 bg-white rounded-2xl border border-slate-200 overflow-hidden">
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            ...
                            <div><label class="field-label">Relationship to Client</label><select class="field">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div><label class="field-label">Total Feeding Days</label><input type="number" min="0"
                                    class="field" placeholder="e.g. 120" id="totalFeedingDays"></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="animate-fade-up-2 bg-white rounded-2xl border border-slate-200 overflow-hidden">
                    <div class="px-4 py-3 border-b border-slate-100 bg-slate-50/60 flex items-center justify-between">
                        <p class="text-[12px] font-semibold text-navy-600">SFP Eligibility Check — Child: Maria Santos
                            Jr.</p>
                        <span
                            class="text-[11px] text-navy-600 font-semibold bg-navy-50 border border-navy-200 px-2.5 py-0.5 rounded-full"><i
                                class="fas fa-check-circle mr-1"></i> Eligible</span>
                    </div>
                    <div class="grid grid-cols-3 divide-x divide-slate-100">
                        <div class="px-5 py-3">
                            <p class="text-[10px] text-slate-400">Current Cycle</p>
                            <p class="text-[13px] font-semibold text-navy-600">Not yet enrolled ✓</p>
                        </div>
                        <div class="px-5 py-3">
                            <p class="text-[10px] text-slate-400">Day Care Capacity</p>
                            <p class="text-[13px] font-semibold text-slate-700">15/30 slots</p>
                        </div>
                        <div class="px-5 py-3">
                            <p class="text-[10px] text-slate-400">Nutrition Status</p>
                            <p class="text-[13px] font-semibold text-navy-500">Underweight (previous)</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="animate-fade-up-3 bg-white rounded-2xl border border-slate-200 overflow-hidden">
                    <div class="flex items-center gap-3 px-6 py-4 border-b border-slate-100 bg-slate-50/60">
                        ...
                        <div>
                            <h2 class="text-[14px] font-semibold text-navy-600">Nutrition Status</h2>
                            <p class="text-[11px] text-slate-400">Based on WHO growth standards</p>
                        </div>
                    </div>
                    <div class="p-6">
                        <div class="grid grid-cols-4 gap-3 mb-5" id="nutriSelector">
                            <div onclick="setNutri(this,'Normal')"
                                class="nutri-opt border-2 border-slate-200 rounded-2xl p-3.5 text-center">
                                <i class="fas fa-check-circle text-navy-600 text-2xl mb-2"></i>
                                <p class="text-[12px] font-semibold text-slate-700">Normal</p>
                            </div>
                            <div onclick="setNutri(this,'Underweight')"
                                class="nutri-opt border-2 border-slate-200 rounded-2xl p-3.5 text-center">
                                <i class="fas fa-exclamation-triangle text-navy-500 text-2xl mb-2"></i>
                                <p class="text-[12px] font-semibold text-slate-700">Underweight</p>
                            </div>
                            <div onclick="setNutri(this,'Severely Underweight')"
                                class="nutri-opt border-2 border-slate-200 rounded-2xl p-3.5 text-center">
                                <i class="fas fa-exclamation-circle text-navy-400 text-2xl mb-2"></i>
                                <p class="text-[12px] font-semibold text-slate-700">Severely Underweight</p>
                            </div>
                            <div onclick="setNutri(this,'Overweight')"
                                class="nutri-opt border-2 border-slate-200 rounded-2xl p-3.5 text-center">
                                <i class="fas fa-chart-bar text-navy-600 text-2xl mb-2"></i>
                                <p class="text-[12px] font-semibold text-slate-700">Overweight</p>
                            </div>
                        </div>
                        <div class="grid grid-cols-3 gap-4">
                            <div><label class="field-label">Weight (kg)</label><input type="number" step="0.1" min="0"
                                    class="field" placeholder="e.g. 14.5"></div>
                            <div><label class="field-label">Height (cm)</label><input type="number" step="0.1" min="0"
                                    class="field" placeholder="e.g. 95.0"></div>
                            <div><label class="field-label">Measurement Date</label><input type="date" class="field">
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function saveComplete() {
            ...
            const feedingDays = document.getElementById('feedingDays');
            if (!feedingDays.value || parseInt(feedingDays.value) < 0) {
                errors.push({ field: feedingDays, msg: 'Please enter valid Feeding Days Completed.' });
            }
